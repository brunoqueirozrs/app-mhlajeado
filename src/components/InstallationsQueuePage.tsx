...
export default function InstallationsQueuePage() {
  ...
  const handleAddProtocol = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newProtocol.protocolo || !newProtocol.vendedor) return;
    ...
    try {
      ...
      if (res.ok) {
        ...
        setNewProtocol({ cliente: '', protocolo: '', vendedor: '', observacoes: '' });
        loadQueue();
      } else {
  ...
  const handleFinalizeSubmit = async () => {
    ...
    try {
      const res = await fetch(
        `/api/installations-queue/${finalizeId}/finalize`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    <div className="space-y-6  font-sans pb-12">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <p className="text-slate-400 font-medium max-w-xl text-sm md:text-base leading-relaxed">
              Monitore os protocolos de instalação gerados pelos vendedores.
              Contate os clientes para validação e garanta a conclusão das
              entregas.
            </p>
          </div>
      ...
      {isAddingProtocol && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 md:p-8 w-full max-w-md shadow-2xl relative">
            <button onClick={() => setIsAddingProtocol(false)} className="absolute top-6 right-6 p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-full transition-colors">
              <X className="w-5 h-5" />
            </button>
            <h3 className="text-xl font-black text-slate-800 mb-2">Adicionar Protocolo</h3>
            <p className="text-sm font-medium text-slate-500 mb-6">Insira os dados do protocolo para monitoramento.</p>
            
            <form onSubmit={handleAddProtocol} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Protocolo *</label>
                <input 
                  type="text"
                  required
                  value={newProtocol.protocolo}
                  onChange={(e) => setNewProtocol({...newProtocol, protocolo: e.target.value})}
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-800 focus:outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-200 transition-all"
                  placeholder="Ex: 20241012345"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Cliente (Opcional)</label>
                <input 
                  type="text"
                  value={newProtocol.cliente}
                  onChange={(e) => setNewProtocol({...newProtocol, cliente: e.target.value})}
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-800 focus:outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-200 transition-all"
                  placeholder="Ex: João da Silva"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Vendedor *</label>
                <select 
                  required
                  value={newProtocol.vendedor}
                  onChange={(e) => setNewProtocol({...newProtocol, vendedor: e.target.value})}
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-800 focus:outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-200 transition-all appearance-none"
                >
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Observações (Opcional)</label>
                <textarea 
                  rows={3}
                  value={newProtocol.observacoes}
                  onChange={(e) => setNewProtocol({...newProtocol, observacoes: e.target.value})}
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-800 focus:outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-200 transition-all resize-none"
                  placeholder="Adicione observações relevantes..."
                />
              </div>
              <div className="pt-4 flex justify-end gap-3">
                <button type="button" onClick={() => setIsAddingProtocol(false)} className="px-5 py-3 text-sm font-bold text-slate-600 hover:text-slate-800 hover:bg-slate-100 rounded-xl transition-colors">
                  Cancelar
                </button>
                <button 
                  type="submit" 
                  disabled={isSubmittingProtocol || !newProtocol.protocolo || !newProtocol.vendedor}
                  className="px-6 py-3 bg-sky-600 hover:bg-sky-500 disabled:bg-sky-400 text-white text-sm font-bold rounded-xl shadow-sm transition-all flex items-center gap-2"
                >
                  {isSubmittingProtocol ? "Adicionando..." : "Adicionar Protocolo"}
                </button>
              </div>
            </form>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {filteredQueue.length === 0 ? (
          <div className="flex flex-col items-center justify-center col-span-full py-16 text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm mt-1">
                Não há instalações com o status selecionado.
              </p>
            </div>
          </div>
        ) : (
            ...
            >
              <div>
                <div className="flex justify-between items-start mb-4">
                  <span
                    className={`px-2.5 py-1 text-[10px] font-black uppercase tracking-widest rounded-lg flex items-center gap-1.5 ${
                      item.status === "Concluido"
                        ? "bg-[#E6FAF1] text-[#00A86B] border border-[#00A86B]/20"
                ...
                <div className="space-y-3 mb-6 bg-slate-50/50 p-4 rounded-2xl border border-slate-100">
                  <div className="flex flex-col gap-1">
                    ...
                    <span className="font-mono text-xs font-bold text-sky-700 leading-relaxed break-words">
                      {item.protocolo}
                    </span>
                  </div>
          ...
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col">
            ...
            <div className="p-6">
              <p className="text-sm font-medium text-slate-500 mb-4">
                Adicione uma observação de fechamento para este protocolo (ex:
                "Instalação concluída com sucesso, cliente satisfeito").
              </p>
              <textarea
