...
async function getExportUrl(spreadsheetId: string, sheetName: string): Promise<string> {
  ...
  if (!gidsCache[spreadsheetId][sheetName.trim().toLowerCase()]) {
    ...
    try {
      ...
      while ((match = regex.exec(text)) !== null) {
        ...
        if (nameMatch && gidMatch) {
          gidsCache[spreadsheetId][nameMatch[1].trim().toLowerCase()] = gidMatch[1];
        }
      }
    } catch (e) {
      console.error("Error fetching GIDs:", e);
    }
  }

  const gid = gidsCache[spreadsheetId][sheetName.trim().toLowerCase()];
  if (gid) {
    return `https://docs.google.com/spreadsheets/d/${spreadsheetId}/export?format=csv&gid=${gid}&_rnd=${Date.now()}`;
  }
  
  return `https://docs.google.com/spreadsheets/d/${spreadsheetId}/gviz/tq?tqx=out:csv&headers=1&sheet=${encodeURIComponent(sheetName)}&_rnd=${Date.now()}`;
}
...
async function syncBaseClientsFromGoogleSheet() {
  ...
  try {
    const url = await getExportUrl("19U8KDUFQUhMOLPIniKCkUfGXZCBY7i3uFyjOQYU003w", "Base052026");
...
async function syncLeadsFromGoogleSheet() {
  ...
  try {
    ...
    // Filter out Frio and Venda Fechada leads AFTER deduplication, so the latest row (which marks it as such) overwrites the active one
    loadedLeads = Array.from(leadMap.values()).filter((l: any) => l.status !== "Frio" && l.status !== "Venda Fechada" && l.status !== "Lead Frio");
...
async function syncFttaFromGoogleSheet() {
  ...
  try {
    ...
    const lajeadoUrl = await getExportUrl("19U8KDUFQUhMOLPIniKCkUfGXZCBY7i3uFyjOQYU003w", "FTTA LAJEADO");
    ...
    const estrelaUrl = await getExportUrl("19U8KDUFQUhMOLPIniKCkUfGXZCBY7i3uFyjOQYU003w", "FTTA ESTRELA");
    ...
    const prospUrl = await getExportUrl("19U8KDUFQUhMOLPIniKCkUfGXZCBY7i3uFyjOQYU003w", "FTTA PROSPECÇÃO");
...
leadsFrios = readJSONDb("leadsFrios.json", []);
...
async function syncCobrancasFromGoogleSheet() {
  ...
  try {
    const signal = AbortSignal.timeout ? AbortSignal.timeout(120000) : undefined;
    const url = await getExportUrl("19U8KDUFQUhMOLPIniKCkUfGXZCBY7i3uFyjOQYU003w", "acao_cobranca");
    const res = await fetch(url, { signal });
...
async function syncInstallationsFromGoogleSheet() {
  ...
  try {
    const url = await getExportUrl("19U8KDUFQUhMOLPIniKCkUfGXZCBY7i3uFyjOQYU003w", "Agenda Instalação");
    const signal = AbortSignal.timeout ? AbortSignal.timeout(120000) : undefined;
...
app.post("/api/n8n/test", async (req, res) => {
  ...
  try {
    ...
    // Fire and forget so we don't wait for n8n to respond fully if it's slow
    fetch(url, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
app.post("/api/env/n8n/restore", async (req, res) => {
...
app.put("/api/leads", async (req, res) => {
  ...
  if (fullUpdatedLead && fullUpdatedLead.status === "Venda Fechada") {
    ...
    // Remover do funil sem deslocar _linha, já que deleteLead/saveLead no Apps Script funciona via append-only ou não afeta linhas originais
    leads = leads.filter(l => String(l._linha) !== String(fullUpdatedLead._linha));
    
    writeJSONDb("leads.json", leads);
    lastLeadsSyncTime = Date.now();
    await writeLeadToGoogleSheet(fullUpdatedLead, "save"); // Atualiza status com append
...
app.delete("/api/leads", (req, res) => {
  ...
  if (leadToDel) {
    ...
    // NOTA: Removido o shift de _linha, pois as linhas não são deslocadas no Sheets quando um delete é tentado via script (ou falha)
    writeJSONDb("leads.json", leads);
    lastLeadsSyncTime = Date.now();
...
let localObjections = readJSONDb("local_objections.json", []);

app.get("/api/matriz-objecoes", async (req, res) => {
  const sheetName = "Matriz Objeções";
  const url = await getExportUrl("19U8KDUFQUhMOLPIniKCkUfGXZCBY7i3uFyjOQYU003w", sheetName);
...
app.get("/api/pos-vendas/:sheetName", async (req, res) => {
  ...
  try {
    ...
    const cobranca1Idx = headers.findIndex((h: string) => h.includes("cobrança 1") || h.includes("cobranca 1") || h.includes("cobraça 1") || h.includes("1° m"));
...
app.post("/api/pos-vendas/disparar-indicacoes", async (req, res) => {
  ...
  let webhookUrl = isTest 
     ...
     : (process.env.N8N_WEBHOOK_URL_INDICACOES || "http://localhost:5678/webhook/indicacoes");
  ...
  try {
    ...
    res.json({ status: "success", message: "Disparo em massa iniciado" });
  } catch (error) {
    console.error("Erro no disparo em massa Indicações:", error);
    res.status(500).json({ status: "error", message: "Falha ao enviar webhook", error: error.message });
  }
});

app.post("/api/pos-vendas/disparar-sva", async (req, res) => {
  const { clientes } = req.body;
  if (!Array.isArray(clientes)) {
    return res.status(400).json({ status: "error", message: "Clientes deve ser um array" });
  }
  ...
  try {
    ...
    console.error("Erro no disparo em massa SVA:", error);
    res.status(500).json({ status: "error", message: "Falha ao enviar webhook", error: error.message });
  }
});
...
function triggerApprovalWebhook(absence) {
  ...
  if (webhookUrl && webhookUrl !== "https://n8n-url-placeholder/webhook/absence-approval" && webhookUrl !== "") {
      const vendorData = vendors.find((v: any) => v.nome.toLowerCase() === absence.vendedor.toLowerCase());
      const payload = {
          vendedor: absence.vendedor,
          telefone: vendorData ? vendorData.telefone : "",
...
app.post("/api/gemini/suggestBasePlanos", async (req, res) => {
  ...
  try {
    ...
    const url = await getExportUrl(SPREADSHEET_ID, "Planos");
...
let localTradeActions: Record<string, any[]> = readJSONDb("local_trade.json", {});
...
app.post("/api/n8n/webhook-vendas-sva", async (req, res) => {
  ...
  try {
    fetch(webhookUrl, {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload)
    }).catch(e => console.error("Webhook vendas-sva failed:", e));
    
    res.json({ success: true, message: "Webhook disparado (fire-and-forget)" });
  } catch (error) {
    console.error("Erro vendas-sva:", error);
    res.status(500).json({ error: "Falha ao enviar webhook" });
  }
});
...
app.post("/api/n8n/webhook-indicacoes", async (req, res) => {
  ...
  console.log("[N8N] Disparando webhook indicacoes para:", webhookUrl);
  
  const historyItem = {
    id: Date.now(),
    timestamp: new Date().toISOString(),
    event: 'indicacoes_disparo',
    url: webhookUrl,
    payload: payload,
    status: 'pending'
  };
  ...
  try {
    fetch(webhookUrl, {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload)
    }).catch(e => console.error("Webhook indicacoes failed:", e));
    
    res.json({ success: true, message: "Webhook disparado (fire-and-forget)" });
  } catch (error) {
    console.error("Erro indicacoes:", error);
    res.status(500).json({ error: "Falha ao enviar webhook" });
  }
});
...
let internalProtocols: any[] = readJSONDb("internalProtocols.json", []);
...
async function syncInstallationQueueFromGoogleSheet() {
  ...
  try {
    const signal = AbortSignal.timeout ? AbortSignal.timeout(30000) : undefined;
    const url = await getExportUrl("19U8KDUFQUhMOLPIniKCkUfGXZCBY7i3uFyjOQYU003w", "Fila de Monitoramento");
