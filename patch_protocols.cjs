...
const syncFunctionRegex = /async function syncInternalProtocolsFromGoogleSheet\(\) \{[\s\S]*?\}\n/m;
const writeFunctionRegex = /async function writeInternalProtocolToGoogleSheet\([\s\S]*?\}\n/m;

const newSyncFunction = `async function syncInternalProtocolsFromGoogleSheet() {
...
const newWriteFunction = `async function writeInternalProtocolToGoogleSheet(item: any, action: "append" | "update" = "append") {
  try {
...
content = content.replace(syncFunctionRegex, newSyncFunction);
content = content.replace(writeFunctionRegex, newWriteFunction);

const putRouteOriginal = `      // We should ideally update Google Sheet as well, but for now local is fine
      // writeInternalProtocolToGoogleSheet(internalProtocols[index], "update");`;
      
const putRouteFixed = `      writeInternalProtocolToGoogleSheet(internalProtocols[index], "update").catch(e => console.error(e));`;
content = content.replace(putRouteOriginal, putRouteFixed);
